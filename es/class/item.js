...
import ReactPlannerService, { Action, ItemTypes } from "../ReactPlannerService";
...
var Item = function () {
  ...
  _createClass(Item, null, [{
    ...
    value: function select(state, layerID, itemID) {
      var isCreatedSelect = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
    ...
    value: function remove(state, layerID, itemID) {
      if (window.confirm("Are you sure want to remove this Place?")) {
        ...
        // Autonomous
        ReactPlannerService.getInstance().onAction(Action.REMOVE, ItemTypes.ITEM, itemID);
      }
    ...
    value: function updateDrawingItem(state, layerID, x, y) {
      if (state.hasIn(["drawingSupport", "currentID"])) {
        ...
        state = Item.select(stateI, layerID, item.id, true).updatedState;
